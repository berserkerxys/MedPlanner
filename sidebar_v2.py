...
def render_sidebar():
    ...
    # IMPORTANTE: Não sobrescrevemos se já existir, para respeitar a navegação do usuário.
    if "sb_meta_slider" not in st.session_state:
        st.session_state.sb_meta_slider = meta_banco
        
    # Garante sincronia inversa: Se o perfil alterou a meta, precisamos refletir aqui se o slider não foi tocado
    # Mas priorizamos o valor local da sessão se ele existir.
    ...
    with st.sidebar:
        ...
        # --- Meta Diária (Slider) ---
        def on_meta_change():
            # 1. Captura novo valor
            novo_valor = st.session_state.sb_meta_slider
            
            # 2. Salva no Banco de Dados
            update_meta_diaria(u, novo_valor)
            
            # 3. Sincroniza com a variável do PERFIL para manter consistência
        ...
        if st.button("🚪 Sair (Logout)", use_container_width=True):
            ...
            # Limpa chaves de sessão para garantir reload limpo
